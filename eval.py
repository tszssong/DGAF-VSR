...
parser.add_argument("out_path", type=str, default='./results/REDS4/dgaf_s50/', help="Path to output folder containing the upscaled frames.")
parser.add_argument("--gt_path", type=str, default='/mnt/nfs0/hanzhang/data/ll/REDS/val4/GT', help="Path to folder with GT frames.")
...
with open(args.out_path.rstrip('/')+'_ret.txt', 'a') as fw:
...
for seq in seqs:
    print(f'{seq}: PSNR={np.mean(psnr_dict[seq]):.3f}, SSIM={np.mean(ssim_dict[seq]):.3f}, LPIPS={np.mean(lpips_dict[seq]):.3f}, DISTS={np.mean(dists_dict[seq]):.3f}, MUSIQ={np.mean(musiq_dict[seq]):.3f}, CLIP={np.mean(clip_dict[seq]):.3f}, NIQE={np.mean(niqe_dict[seq]):.3f}, tLPIPS={1e3*np.mean(tlpips_dict[seq]):.3f}, tOF={1e1*np.mean(tof_dict[seq]):.3f}')
    with open(args.out_path.rstrip('/')+'_ret.txt', 'a') as fw:
        fw.write(f'{seq}: PSNR={np.mean(psnr_dict[seq]):.3f}, SSIM={np.mean(ssim_dict[seq]):.3f}, LPIPS={np.mean(lpips_dict[seq]):.3f}, DISTS={np.mean(dists_dict[seq]):.3f}, MUSIQ={np.mean(musiq_dict[seq]):.3f}, CLIP={np.mean(clip_dict[seq]):.3f}, NIQE={np.mean(niqe_dict[seq]):.3f}, tLPIPS={1e3*np.mean(tlpips_dict[seq]):.3f}, tOF={1e1*np.mean(tof_dict[seq]):.3f}\n')
