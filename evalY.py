...
from examples.dgafvsr.util.flow_utils import get_flow
import argparse
import warnings
warnings.filterwarnings("ignore")

parser = argparse.ArgumentParser(description="Evaluation code for StableVSR.")
parser.add_argument("out_path", type=str, default='./results/Vid4/dgaf_s50/', help="Path to output folder containing the upscaled frames.")
parser.add_argument("--gt_path", type=str, default='/mnt/nfs0/hanzhang/data/ll/Vid4/GTr', help="Path to folder with GT frames.")
...
for seq in seqs:
    ...
    for i, (im_rec, im_gt) in enumerate(zip(ims_rec, ims_gt)):
        with torch.no_grad():
            ...
            gt_cv = cv2.cvtColor(np.asarray(gt),cv2.COLOR_RGB2BGR) 
            lq_cv = cv2.cvtColor(np.asarray(rec),cv2.COLOR_RGB2BGR) 
            bgr_h, bgr_w, bgr_c = gt_cv.shape
            yuv_gt = cv2.cvtColor(gt_cv, cv2.COLOR_BGR2YUV_I420)
            yuv_h, yuv_w = yuv_gt.shape
            assert bgr_h == yuv_h*2//3
            gt_y = yuv_gt[:bgr_h, :]
            yuv_lq = cv2.cvtColor(lq_cv, cv2.COLOR_BGR2YUV_I420)
            rec_y = yuv_lq[:bgr_h, :]
            
            gt_y3 = np.stack([gt_y, gt_y, gt_y], axis = 2)
            rec_y3 = np.stack([rec_y, rec_y, rec_y], axis = 2)
            # print(gt_y3.shape, gt_y.shape, gt.size)
            # gt_y = gt.convert('L')
            # rec_y = rec.convert('L')
            gt = tt(gt).unsqueeze(0).to(device)
            rec = tt(rec).unsqueeze(0).to(device)
            gt_y = tt(gt_y).unsqueeze(0).to(device)
            rec_y = tt(rec_y).unsqueeze(0).to(device)
            gt_y3 = tt(gt_y3).unsqueeze(0).to(device)
            rec_y3 = tt(rec_y3).unsqueeze(0).to(device)

            # psnr_value = psnr(gt, rec)
            # ssim_value = ssim(gt, rec)
            psnr_value = psnr(gt_y, rec_y)
            ssim_value = ssim(gt_y, rec_y)
            # lpips_value = lpips(gt, rec)
            lpips_value = lpips(gt_y3, rec_y3)
            # dists_value = dists(gt, rec)
            dists_value = dists(gt_y, rec_y)
...
with open(args.out_path.rstrip('/')+'_retY.txt', 'a') as fw:
    fw.write(f'PSNR: {mean_psnr}, SSIM: {mean_ssim}, LPIPS: {mean_lpips}, DISTS: {mean_dists}, MUSIQ: {mean_musiq}, CLIP: {mean_clip}, NIQE: {mean_niqe}, tLPIPS: {mean_tlpips}, tOF: {mean_tof}\n')
    
    
for seq in seqs:
    print(f'{seq}: PSNR={np.mean(psnr_dict[seq]):.4f}, SSIM={np.mean(ssim_dict[seq]):.4f}, LPIPS={np.mean(lpips_dict[seq]):.4f}, DISTS={np.mean(dists_dict[seq]):.4f}, MUSIQ={np.mean(musiq_dict[seq]):.4f}, CLIP={np.mean(clip_dict[seq]):.4f}, NIQE={np.mean(niqe_dict[seq]):.4f}, tLPIPS={1e3*np.mean(tlpips_dict[seq]):.4f}, tOF={1e1*np.mean(tof_dict[seq]):.4f}')
    with open(args.out_path.rstrip('/')+'_retY.txt', 'a') as fw:
        fw.write(f'{seq}: PSNR={np.mean(psnr_dict[seq]):.3f}, SSIM={np.mean(ssim_dict[seq]):.4f}, LPIPS={np.mean(lpips_dict[seq]):.4f}, DISTS={np.mean(dists_dict[seq]):.4f}, MUSIQ={np.mean(musiq_dict[seq]):.4f}, CLIP={np.mean(clip_dict[seq]):.4f}, NIQE={np.mean(niqe_dict[seq]):.4f}, tLPIPS={1e3*np.mean(tlpips_dict[seq]):.4f}, tOF={1e1*np.mean(tof_dict[seq]):.4f}\n')
