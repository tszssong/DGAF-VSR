...
from diffusers import DGAFFeaAlignPipeline
from diffusers import DDPMScheduler, DGAFNetModel
...
# get arguments
parser = argparse.ArgumentParser(description="Test code for DGAF-VSR.")
parser.add_argument("--model_id", default='./DGAFVSR_results/', type=str, help="Path to output folder.")
parser.add_argument("--out_path", default='./DGAFVSR_results/', type=str, help="Path to output folder.")
...
parser.add_argument("--ckpt", type=str, default=None, help="Path to your folder with the controlnet checkpoint.")
...
dgafnet_model = DGAFNetModel.from_pretrained(args.ckpt) 
pipeline = DGAFFeaAlignPipeline.from_pretrained(args.model_id, dgafnet=dgafnet_model)
...
for seq in seqs:
    ...
    # save_path = os.path.join(args.out_path, f"{seq}_debug")
    # os.makedirs(save_path, exist_ok=True)
    # frames = pipeline(save_path, frames, num_inference_steps=args.num_inference_steps, guidance_scale=0, of_model=of_model, debug=True).images
...
print(f"Results in {args.out_path}")