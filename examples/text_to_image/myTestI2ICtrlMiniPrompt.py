...
if __name__ == "__main__":
    ...
    parser.add_argument('--s_i2i', type=float, default=0.4, help='')
    parser.add_argument('--s_ctl', type=float, default=0.6, help='')
    ...
    args.out = os.path.join(args.lora, f"{prefix}_i2i{args.s_i2i}_depth_anything{args.s_ctl}_miniPrompt_{args.inw}x{args.inh}")
    ...
    with open(args.lst, 'r') as fr:
        ...
        for idx, line in enumerate(content_lists):
            ...
            with open(text_path, 'r') as fr:
                caption = fr.readline()
            if 'dog' in caption:
                caption = 'xhs cartoon, a dog' 
            else:
                caption = 'xhs cartoon, a cat' #+ ', big eyes, happy'
