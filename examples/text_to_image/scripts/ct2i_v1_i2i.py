...
def get_pipe():
    ...
    pipe = pipe.to("cuda")
    return pipe
def filter_cpation(text):
    if 'illustration' in text: return True
    elif 'elderly' in text: return True
    elif 'old' in text: return True
    elif 'older' in text: return True
    elif 'painting' in text: return True
    elif 'middle-aged' in text: return True
    elif 'black and white' in text: return True
    else: return False
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lst', type=str, default='/mnt/nfs0/hanzhang/data/style/t2i/vcg/vcg-00000-img_human2k.lst', help='')
    ...
    subpth=f"cDisneyVCGHA5k_{prefix}_{args.fvision}_{args.inw}x{args.inh}seed{args.seed}steps{args.steps}"
    ...
    caption_pre = "masterpiece, best quality, 8k, official art, cinematic light, ultra high res, "
    caption_post = ''
    ...
    with open(args.lst, 'r') as fr:
        ...
        for idx, line in enumerate(content_lists):
            ...
            with open(tpth, 'r') as fr:
                caption = fr.readline()
            if filter_cpation(caption): 
                logging.info(f"{caption} skipped!")
                continue
            ...
            opth = os.path.join(args.out, f'disney_{idx:04d}_0905.png')
            ret.save(opth)
