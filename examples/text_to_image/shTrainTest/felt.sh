...
DATASET_NAME=/mnt/nfs0/hanzhang/data/style/xhs_felt/train30
size=960
bs=1
OUT_PTH=/mnt/nfs1/hanzhang/LoRA/felt/${size}x${bs}_$(date "+%m%d-%H%M%S")
OUT_PTH_SCRIPTS=${OUT_PTH}/scripts
mkdir ${OUT_PTH}
mkdir ${OUT_PTH_SCRIPTS}
cp -r $(dirname $0) ${OUT_PTH_SCRIPTS}
CUDA_VISIBLE_DEVICES='1' accelerate launch train_text_to_image_lora_sdxl.py \
  ...
  --seed=42 \
  --output_dir=$OUT_PTH \
  --validation_prompt="xhs felt, a cat, gray and white, blue eyes, lay, hardwood floor"
