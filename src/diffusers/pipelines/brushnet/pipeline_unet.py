...
class BrushUnetPipeline(
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None:
            ...
            # print(f"Hz: You choose to use an init latent/noise, we make the following modifications:")
            # print(f"dst dtype:{dtype}, input latent dtype:{latents.dtype}")    # torch.float16  
            # print(f"dst shape:{shape}, input latent shape:{latents.shape}")    # 1, 4, 128, 128
    ...
    def __call__(
        ...
        save_path=None,     # hz. debug mode, save latents or intermeidiate results to save_path
        **kwargs,
    ):
        r"""
        ...
        if not save_path is None:               # debug mode
            import cv2
            print(f"Hz: save path={save_path}") 
            b, c, h, w = latents.shape
            for cidx in range(c):
                l = latents[0][cidx]
                l = 255.*(l-torch.min(l))/(torch.max(l)-torch.min(l))
                l = l.cpu().numpy()
                l = l.astype(np.uint8)
                cv2.imwrite(save_path+f'_{cidx}.png', l)
