...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        print(f"{datetime.datetime.now().strftime(ISOTIMEFORMAT)}hz. latents:{latents.shape}")
        print(f"latents:{latents[0,0,0,:10,:10]}")
        if not output_type == "latent":
            # cast back to fp16 if needed
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
            frames = self.decode_latents(latents, num_frames, decode_chunk_size)
            print(f"{datetime.datetime.now().strftime(ISOTIMEFORMAT)}hz. frames:{frames.shape}")
            print(f"frames:{frames[0,0,0,:10,:10]}")
            frames = tensor2vid(frames, self.image_processor, output_type=output_type)
            print(f"{datetime.datetime.now().strftime(ISOTIMEFORMAT)}hz. frames:{len(frames[0])}")
        else:
            frames = latents
